...
async def manualverify(ctx):
    if ctx.message.channel.id not in ch_id2ch:
        await ctx.send(
            "This is not a valid bot commands channel. Please issue commands in one of"
            f" {nicelist(ch_n2id.keys())}."
        )
        return
    if not (
        set([get_role("hall-chair"), get_role("moderator")])
        and set(ctx.message.author.roles)
    ):
        await ctx.send("Only hall chairs and moderators can manually verify users.")
        return

    mentions = ctx.message.mentions
    if len(mentions) != 1:
        await ctx.send(
            "You must mention (tag with the @ symbol) exactly one user to assign the"
            " roles to."
        )
        return
    ...
    await ctx.send(
        f"Granted verified role to {user.mention}. \nIf you'd like to undo this or add"
        " different/multiple hall roles, contact a moderator."
    )
...
async def selfrole(ctx, role_str=None):
    ...
    if get_role("verified") not in ctx.message.author.roles:
        await ctx.send(
            "You haven't yet verified your MIT email. First acquire the `verified`"
            " role, then try again."
        )
        return
    if role_str is None:
        await ctx.send("Please name a role to add.")
        return

    if len(ctx.message.role_mentions) > 0:
        await ctx.send(
            "Please do not mention your desired role - just enter its name :)"
        )
        return
    ...
    if rq_role is None:
        await ctx.send(
            f"{role_str.replace('@','')} is not a valid role. Allowed selfroles are"
            f" {nicelist(allowed_selfroles)}."
        )
        return
    elif role_str not in allowed_selfroles:
        await ctx.send(
            f"{role_str.replace('@','')} is not an allowed selfrole. Allowed selfroles"
            f" are {nicelist(allowed_selfroles)}."
        )
        return
...
async def addaffiliate(ctx):
    ...
    if get_role("ec-resident") not in ctx.message.author.roles:
        await ctx.send("Only EC residents can add new EC affiliates.")
        return

    caller_hall_role_set = set(ctx.message.author.roles) & hall_roles()
    if len(caller_hall_role_set) != 1:
        await ctx.send(
            f"You have {len(caller_hall_role_set)} hall roles, which is very confusing"
            f" to joeg. Please contact a {get_role('moderator').mention} for manual"
            " resolution."
        )
        return
    ...
    if get_role("verified") not in user.roles:
        await ctx.send(
            f"{user.mention} hasn't yet verified their MIT email. Ask them to first"
            " acquire the `verified` role via `!verify`, then try again."
        )
        return
    ...
    await ctx.send(
        f"Granted ec-affiliated and {caller_hall_role.name[-3:-1]} roles to"
        f" {user.mention}. \nIf you'd like to undo this or add different/multiple hall"
        " roles, contact a moderator."
    )
...
async def addresident(ctx):
    ...
    mentions = ctx.message.mentions
    if len(mentions) != 1:
        await ctx.send("You must mention exactly one user to assign the roles to.")
        return
    ...
    await user.add_roles(caller_hall_role)
    await ctx.send(
        f"Granted ec-affiliated, ec-resident, and {caller_hall_role.name[-3:-1]} roles"
        f" to {user.mention}. \nIf you'd like to undo this or add different/multiple"
        " hall roles, contact a moderator."
    )
...
async def addhallmod(ctx):
    ...
    if not (
        ...
        await ctx.send(
            "Only hall chairs and server moderators can add new hall moderators."
        )
        return
    ...
    if get_role("ec-resident") not in user.roles:
        await ctx.send("Please use !addresident first.")
        return
    await user.add_roles(get_role("hall-moderator"))
    await ctx.send(
        f"Granted hall-moderator role to {user.mention}. \nIf you'd like to undo this,"
        " contact a moderator."
    )
...
async def on_command_error(ctx, error):
    ...
    await ctx.send(
        f"You've encountered an error: {errstr}"
